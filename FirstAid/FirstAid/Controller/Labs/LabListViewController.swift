...
class LabListViewController: UIViewController {
    ...
    func getLabs() {
        ...
        DataProvider.sharedInstance.getDataUsingHeaderAndGet(path: Webservice.selectLab + User.location! + "&" + testStr, headers: headers, { (json) in
    ...
    func addItem() {
        ...
        for test in selectedTestArr {
           // skus.updateValue(test, forKey: "test_id")
            skus.append([
                "test_id": test,
    ...
    @IBAction func labAction(_ sender: UIButton) {
       ...
       // let nextView = storyboard.instantiateViewController(withIdentifier: "LabCartViewController") as! LabCartViewController
