...
class LabsViewController: UIViewController {
    ...
    func getLabTest() {
        ...
        DataProvider.sharedInstance.getDataUsingHeaderAndGet(path: Webservice.getPopularTest + replaced + "&popular_test_size=5&packages_page_size=3", headers: headers, { (json) in
            print(json)
...
extension LabsViewController: FeatureLabDelegate {
    func featureLab(id: String) {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let nextview = storyboard.instantiateViewController(withIdentifier: "FeatureLabsViewController") as! FeatureLabsViewController
        nextview.labId = id
        self.navigationController?.pushViewController(nextview, animated: true)
    }
}
