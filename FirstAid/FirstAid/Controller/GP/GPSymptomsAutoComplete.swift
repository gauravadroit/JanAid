...
class GPSymptomsAutoComplete: UIViewController {
    ...
    func getSearchData(name:String) {
        ...
        request =  Alamofire.request(Webservice.getSymptom, method: .post, parameters:parameter, encoding: URLEncoding.default, headers:Webservice.header).responseJSON { response in
...
extension GPSymptomsAutoComplete:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if((self.delegate) != nil)
        {
            self.view.endEditing(true)
            self.dismiss(animated: true, completion: nil)
            delegate?.symptomValue(self.symArr[indexPath.row]);
        }
    }
    
}

extension GPSymptomsAutoComplete: UITextFieldDelegate {
