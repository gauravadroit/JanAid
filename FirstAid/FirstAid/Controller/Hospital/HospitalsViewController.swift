...
class HospitalsViewController: UIViewController {
    ...
    @objc func directionAction(_ sender: UIButton) {
        ...
        nextView.lat = hospitalArr[sender.tag].lat
        nextView.long = hospitalArr[sender.tag].long
        nextView.hosiptalName = hospitalArr[sender.tag].name
        self.navigationController?.pushViewController(nextView, animated: true)
    }
    ...
    func getHospital() {
        ...
        //DataProvider.sharedInstance.getDataUsingPost(path: Webservice.getHospital, dataDict: parameter, { (json) in
            print(json)
